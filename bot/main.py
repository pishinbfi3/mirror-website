...
def main():
    print("🤖 Bale SSH Bot starting (enhanced version)...")
    setup_environment()
    cleanup_old_files()

    try:
        config = BotConfig.from_env()
        print(f"✅ Config loaded – Chat ID: {config.chat_id[:10]}...")
    except ValueError as e:
        print(f"❌ Config error: {e}")
        sys.exit(1)

    handler = BaleBotHandler(config)
    print("✅ Bot handler initialized with persistent shell")
    ...
    while not stop_requested:
        ...
        for update in updates:
            ...
            if str(chat.get("id")) != config.chat_id:
                print(f"⚠️ Ignoring chat {chat.get('id')}")
                continue
